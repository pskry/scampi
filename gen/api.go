...
func (g *apiGenerator) header() {
	g.line(
		"// Generated by scampi gen api (%s) from %s %s",
		g.scampiVersion,
		g.doc.Info.Title,
		g.doc.Info.Version,
	)
...
func (g *apiGenerator) writeOperation(path, method string, op *openapi3.Operation) {
	...
	if op.Summary != "" {
		g.line("// %s %s — %s", method, path, op.Summary)
	} else {
		g.line("// %s %s", method, path)
	}
...
func groupByPrefix(paths []string) []pathGroup {
	...
	for _, p := range paths {
		// Strip path params ({id}, {hostID}, etc.) before grouping
		// so /audit-log/{id} stays under "audit-log", not "{id}".
		stripped := stripPathParams(strings.TrimPrefix(p, "/"))
...
// stripPathParams removes {param} segments from a path so grouping
// isn't fragmented by path parameters. "/audit-log/{id}" → "audit-log".
